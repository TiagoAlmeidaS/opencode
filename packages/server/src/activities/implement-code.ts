...
function buildTask(
  ...
  parts.push(
    ...
    ``,
    `RULE 5 — CONTAINER ENVIRONMENT (no docker, no sudo):`,
    `You are running INSIDE a container. docker, docker compose, docker-compose, and sudo are NOT available.`,
    `Do NOT attempt to start services with docker. Use in-memory/SQLite alternatives, mock the service,`,
    `or skip integration tests that require external services (databases, Redis, etc.).`,
    `Available tools: git, bun, node, npm, pnpm, yarn, curl, bash.`,
    ``,
