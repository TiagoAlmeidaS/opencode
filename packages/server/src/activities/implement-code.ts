...
function buildTask(
  ...
  parts.push(
    ...
    ``,
    `RULE 5 — CONTAINER ENVIRONMENT:`,
    `You are running INSIDE a container. docker, docker compose, and sudo are NOT available — do not use them.`,
    `Pre-installed: git, bun, node, npm, pnpm, yarn, curl, bash, mise.`,
    ``,
    `For any runtime NOT pre-installed (Python, Java, Go, Rust, Ruby, PHP, .NET, etc.), use mise:`,
    `  mise install python@latest      # or python@3.12, java@21, go@latest, rust@latest, ruby@3.3, php@8.3`,
    `  mise use --global <runtime>@<version>   # activate globally for this session`,
    `  mise exec -- <command>                  # run a single command with the runtime active`,
    `mise reads .tool-versions, .mise.toml, pyproject.toml, go.mod, pom.xml automatically.`,
    `For services (databases, Redis, etc.): use in-memory/SQLite alternatives or skip those integration tests.`,
    ``,
